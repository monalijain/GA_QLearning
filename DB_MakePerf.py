...
def Make_Performance_Measures(Begin,End,Train_Or_Report,walkforward_number):
    ...
    resultDates = dbObject1.dbQuery("SELECT DISTINCT(date),1 FROM "+ gv.priceSeriesTable +" WHERE date >= '" + Begin + "' AND date <= '"+End+"'")
    #dbObject2.dbClose()
    ...
    query= "SELECT *, 1 FROM " + gv.name_Tradesheet_Table + " WHERE entry_date >= '"+Begin + "' AND entry_date <= '"+End+"'"
